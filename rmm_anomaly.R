...
mu1=matrix(mu1,nrow=length(w1))
mu2=matrix(mu2,nrow=length(w2))
sigma1=matrix(sigma1,nrow=length(w1))
sigma2=matrix(sigma2,nrow=length(w2))
if (ncol(mu2)>ncol(mu1)) mu1=cbind(mu1,matrix(NaN,nrow=nrow(mu1),ncol=(ncol(mu2)-ncol(mu1))))
if (ncol(sigma2)>ncol(sigma1)) sigma1=cbind(sigma1,matrix(NaN,nrow=nrow(sigma1),ncol=(ncol(sigma2)-ncol(sigma1))))
w_adapt=w1
mu_adapt=mu1
sigma_adapt=sigma1
if (length(v1)!=0) v_adapt=v1
...
for (i in 1:length(w1)){
	...
	mu_diff=sqrt(rowSums((mu2[,is.na(mu1[i,])==0]-repmat(mu1[i,is.na(mu1[i,])==0],nrow(mu2),1))^2))
	while (1){
		if (length(mu_diff)==0) break
		index=which.min(mu_diff)
		# F test of the equality of two covariance matrices
		# test1=F_test_covariance(sigma1[i,is.na(sigma1[i,])==0],sigma2[index1[index],is.na(sigma1[i,])==0],w1[i]*N1,w2[index1[index]]*N2)
		# Hotelling's T-square test of two sample means with equal covariance matrices
		test2=Hotel_T2_test_mean(mu1[i,is.na(mu1[i,])==0],mu2[index1[index],is.na(mu1[i,])==0],sigma1[i,is.na(sigma1[i,])==0],sigma2[index1[index],is.na(sigma1[i,])==0],w1[i]*N1,w2[index1[index]]*N2)
		if (is.na(test2)) {
			index1=index1[-index]
	...
	mu_adapt[i,is.na(mu_adapt[i,])]=mu2[index1,is.na(mu_adapt[i,])]
	sigma_adapt[i,]=abs(alpha[3]*(sigma2[index1,]+mu2[index1,]^2)+(1-alpha[3])*(sigma1[i,]+mu1[i,]^2)-mu_adapt[i,]^2)
	sigma_adapt[i,is.na(sigma_adapt[i,])]=sigma2[index1,is.na(sigma_adapt[i,])]
...
# normalization
w_adapt=matrix(w_adapt[is.na(rowSums(mu_adapt))==0],ncol=1)
if (length(v1)!=0) v_adapt=matrix(v_adapt[is.na(rowSums(mu_adapt))==0],ncol=1)
sigma_adapt=matrix(sigma_adapt[is.na(rowSums(mu_adapt))==0,],nrow=length(w_adapt))
mu_adapt=matrix(mu_adapt[is.na(rowSums(mu_adapt))==0,],nrow=length(w_adapt))
...
if (method==2){
	for (k in 1:length(w)){
		...
		xbar_a1=repmat(sum_prob1a,1,ncol(x_a))*abs(x_a-repmat(E_q,nrow(x_a),1))/sqrt(repmat(sigma_S,nrow(x_a),1))
		xbar_a[is.na(xbar_a1)==0 & is.infinite(xbar_a1)==0]=xbar_a[is.na(xbar_a1)==0 & is.infinite(xbar_a1)==0]+xbar_a1[is.na(xbar_a1)==0 & is.infinite(xbar_a1)==0]
	}
...
while (split_index==1){
	...
	if (split_lev==1){
		...
		clust=kmeans(x,centers,iter.max=50,algorithm="MacQueen", trace=FALSE)}
	centers=clust$centers
	clust_index=0
	for (i in sort(unique(clust$cluster),decreasing=FALSE)){
		if (length(x[clust$cluster==i,1])<0.001*nrow(x)) next
		# tryCatch(clust1=kmeans(x[clust$cluster==i,],2,iter.max=100,algorithm="MacQueen", trace=FALSE),error=function(e) e, finally=next)
		tryerror=try(kmeans(x[clust$cluster==i,],2,iter.max=100,algorithm="MacQueen", trace=FALSE),silent=FALSE)
		if (length(tryerror)>1) clust1=tryerror
		else next
		# clust1=kmeans(x[clust$cluster==i,],2,iter.max=100,algorithm="MacQueen", trace=FALSE)
		v=clust1$centers[1,]-clust1$centers[2,]
		if (length(x[clust$cluster==i,1])<=7) next
		x_v=matrix(rowSums(x[clust$cluster==i,]*repmat(v,length(x[clust$cluster==i,1]),1))/sum(v^2),ncol=1)
...
for (i in unique(clust$cluster)){
	...
	if (sum(c(is.na(w[index]),is.na(mu[index,]),is.na(sigma[index,])))!=0) {
		w=w[-index]
		mu=mu[-index,]
		sigma=sigma[-index,]
	} else index=index+1
...
# [Charles Zaiontz (dated 30 Jan 2016), Statistics using Excel, 2016. URL: http://www.real-statistics.com/.]
...
# [Hamerly (2004), Learning the k in k-means]
gamma_o=Gmeans_clust(x,1e-4)
w=gamma_o$w
mu=gamma_o$mu
sigma=gamma_o$sigma
mu=matrix(mu,nrow=length(w))
sigma=matrix(sigma,nrow=length(w))
...
y$x=y$x/(sqrt(nrow(x)-1)*repmat(y$sdev,nrow(y$x),1))
return(y)}
...
v=D$v[,2:length(D$d)]
v=matrix(v[,lambda>(0.1*lambda[1])],nrow=nrow(v))
...
tao=quantile(a_ij,probs=0.99,na.rm=TRUE) 
a_ij[a_ij<=tao]=0
a_ij[a_ij>tao]=1
diag(a_ij)=0
